...
class NameShop(StateData.StateData):
    ...
    def enter(self, toon, usedNames, warp):
        ...
        if not (self.addedGenderSpecific) or self.oldBoy != self.boy:
            ...
            if not self.addedGenderSpecific:
                ...
                self.namePanel = DirectFrame(parent = aspect2d, image = None, relief = 'flat', state = 'disabled', pos = (-0.41999999999999998, 0, -0.14999999999999999), image_pos = (0, 0, 0.025000000000000001), frameColor = (1, 1, 1, 0.29999999999999999))
                panel = nameShopGui.find('**/tt_t_gui_mat_namePanel')
                self.panelFrame = DirectFrame(image = panel, scale = (0.75, 0.69999999999999996, 0.69999999999999996), relief = 'flat', frameColor = (1, 1, 1, 0), pos = (-0.016299999999999999, 0, 0.11990000000000001))
                ...
                self.titleCheck = self.makeCheckBox((-0.61499999999999999, 0, 0.371), TTLocalizer.TitleCheckBox, (0, 0.25, 0.5, 1), self.titleToggle)
                self.firstCheck = self.makeCheckBox((-0.21929999999999999, 0, 0.371), TTLocalizer.FirstCheckBox, (0, 0.25, 0.5, 1), self.firstToggle)
            ...
            self.titleScrollList = self.makeScrollList(None, (-0.59999999999999998, 0, 0.20200000000000001), (1, 0.80000000000000004, 0.80000000000000004, 1), self.allTitles, self.makeLabel, [
                TextNode.ACenter,
                'title'])
            self.firstnameScrollList = self.makeScrollList(None, (-0.20000000000000001, 0, 0.20200000000000001), (0.80000000000000004, 1, 0.80000000000000004, 1), self.allFirsts, self.makeLabel, [
                TextNode.ACenter,
                'first'])
    ...
    def makeScrollList(self, gui, ipos, mcolor, nitems, nitemMakeFunction, nitemMakeExtraArgs):
        ...
        it = nitems[:]
        ds = DirectScrolledList(items = it, itemMakeFunction = nitemMakeFunction, itemMakeExtraArgs = nitemMakeExtraArgs, parent = aspect2d, relief = None, command = self._NameShop__listsChanged, pos = ipos, scale = 0.59999999999999998, incButton_image = (self.arrowUp, self.arrowDown, self.arrowHover, self.arrowUp), incButton_relief = None, incButton_scale = (1.2, 1.2, -1.2), incButton_pos = (0.0189, 0, -0.53349999999999997), incButton_image0_color = mcolor, incButton_image1_color = mcolor, incButton_image2_color = mcolor, incButton_image3_color = Vec4(1, 1, 1, 0), decButton_image = (self.arrowUp, self.arrowDown, self.arrowHover, self.arrowUp), decButton_relief = None, decButton_scale = (1.2, 1.2, 1.2), decButton_pos = (0.0195, 0, 0.1779), decButton_image0_color = mcolor, decButton_image1_color = mcolor, decButton_image2_color = mcolor, decButton_image3_color = Vec4(1, 1, 1, 0), itemFrame_pos = (-0.20000000000000001, 0, 0.028000000000000001), itemFrame_scale = 1.0, itemFrame_relief = DGG.RAISED, itemFrame_frameSize = (-0.070000000000000007, 0.5, -0.52000000000000002, 0.12), itemFrame_frameColor = mcolor, itemFrame_borderWidth = (0.01, 0.01), numItemsVisible = 5, forceHeight = TTLocalizer.NSdirectScrolleList)
        return ds

    
    def makeCheckBox(self, npos, ntex, ntexcolor, comm):
        return DirectCheckButton(parent = aspect2d, relief = None, scale = 0.10000000000000001, boxBorder = 0.080000000000000002, boxImage = self.circle, boxImageScale = 4, boxImageColor = VBase4(0, 0.25, 0.5, 1), boxRelief = None, pos = npos, text = ntex, text_fg = ntexcolor, text_scale = TTLocalizer.NSmakeCheckBox, text_pos = (0.20000000000000001, 0), indicator_pos = (-0.56666700000000003, 0, -0.044999999999999998), indicator_image_pos = (-0.26000000000000001, 0, 0.074999999999999997), command = comm, text_align = TextNode.ALeft)

    
    def makeHighlight(self, npos):
        return DirectFrame(parent = aspect2d, relief = 'flat', scale = (0.55200000000000005, 0, 0.11), state = 'disabled', frameSize = (-0.070000000000000007, 0.52000000000000002, -0.5, 0.10000000000000001), borderWidth = (0.01, 0.01), pos = npos, frameColor = (1, 0, 1, 0.40000000000000002))
    ...
    def load(self):
        ...
        self.typeNamePanel = DirectFrame(parent = aspect2d, image = None, relief = 'flat', scale = (0.75, 0.69999999999999996, 0.69999999999999996), state = 'disabled', pos = (-0.016333299999999999, 0, 0.074999999999999997), image_pos = (0, 0, 0.025000000000000001), frameColor = (1, 1, 1, 0))
        self.typePanelFrame = DirectFrame(image = typePanel, relief = 'flat', frameColor = (1, 1, 1, 0), pos = (-0.0080000000000000002, 0, 0.019))
        ...
        self.nameLabel = OnscreenText.OnscreenText(TTLocalizer.PleaseTypeName, parent = aspect2d, style = OnscreenText.ScreenPrompt, scale = TTLocalizer.NSnameLabel, pos = (-0.016333299999999999, 0.53000000000000003))
        self.nameLabel.wrtReparentTo(self.typeNamePanel, sort = 2)
        self.typeANameGUIElements.append(self.nameLabel)
        self.typeNotification = OnscreenText.OnscreenText(TTLocalizer.AllNewNames, parent = aspect2d, style = OnscreenText.ScreenPrompt, scale = TTLocalizer.NStypeNotification, pos = (-0.016333299999999999, 0.14999999999999999))
        self.typeNotification.wrtReparentTo(self.typeNamePanel, sort = 2)
        self.typeANameGUIElements.append(self.typeNotification)
        self.nameMessages = OnscreenText.OnscreenText(TTLocalizer.NameMessages, parent = aspect2d, style = OnscreenText.ScreenPrompt, scale = 0.059999999999999998, pos = (-0.016333299999999999, -0.050000000000000003))
        self.nameMessages.wrtReparentTo(self.typeNamePanel, sort = 2)
        self.typeANameGUIElements.append(self.nameMessages)
        self.nameEntry = DirectEntry(parent = aspect2d, relief = None, scale = TTLocalizer.NSnameEntry, entryFont = getToonFont(), width = MAX_NAME_WIDTH, numLines = 2, focus = 0, cursorKeys = 1, pos = (0.0, 0.0, 0.39000000000000001), text_align = TextNode.ACenter, command = self._NameShop__typedAName, autoCapitalize = 1)
        self.nameEntry.wrtReparentTo(self.typeNamePanel, sort = 2)
        self.typeANameGUIElements.append(self.nameEntry)
        self.submitButton = DirectButton(parent = aspect2d, relief = None, image = (self.squareUp, self.squareDown, self.squareHover, self.squareUp), image_scale = (1.2, 0, 1.1000000000000001), pos = (-0.01, 0, -0.25), text = TTLocalizer.NameShopSubmitButton, text_scale = 0.059999999999999998, text_pos = (0, -0.02), command = self._NameShop__typedAName)
        ...
        self.randomButton = DirectButton(parent = aspect2d, relief = None, image = (self.squareUp, self.squareDown, self.squareHover, self.squareUp), image_scale = (1.1499999999999999, 1.1000000000000001, 1.1000000000000001), scale = (1.05, 1, 1), pos = (0, 0, -0.25), text = TTLocalizer.ShuffleButton, text_scale = 0.059999999999999998, text_pos = (0, -0.02), command = self._NameShop__randomName)
        self.pickANameGUIElements.append(self.randomButton)
        self.typeANameButton = DirectButton(parent = aspect2d, relief = None, image = (self.squareUp, self.squareDown, self.squareHover, self.squareUp), image_scale = (1, 1.1000000000000001, 0.90000000000000002), pos = (0.0033, 0, -0.38833000000000001), scale = (1.2, 1, 1.2), text = TTLocalizer.TypeANameButton, text_scale = TTLocalizer.NStypeANameButton, text_pos = TTLocalizer.NStypeANameButtonPos, command = self._NameShop__typeAName)
        ...
        self.nameResult = DirectLabel(parent = aspect2d, relief = None, scale = TTLocalizer.NSnameResult, pos = (0.0050000000000000001, 0, 0.58499999999999996), text = ' \n ', text_scale = 0.80000000000000004, text_align = TextNode.ACenter, text_wordwrap = MAX_NAME_WIDTH)
        ...
        self.allSuffixes = [
            ...
            ' '] + [
            ' ']
        self.titleScrollList = self.makeScrollList(gui, (-0.59999999999999998, 0, 0.20200000000000001), (1, 0.80000000000000004, 0.80000000000000004, 1), self.allTitles, self.makeLabel, [
            TextNode.ACenter,
            'title'])
        self.firstnameScrollList = self.makeScrollList(gui, (-0.20000000000000001, 0, 0.20200000000000001), (0.80000000000000004, 1, 0.80000000000000004, 1), self.allFirsts, self.makeLabel, [
            TextNode.ACenter,
            'first'])
        self.lastprefixScrollList = self.makeScrollList(gui, (0.20000000000000001, 0, 0.20200000000000001), (0.80000000000000004, 0.80000000000000004, 1, 1), self.allPrefixes, self.makeLabel, [
            TextNode.ARight,
            'prefix'])
        self.lastsuffixScrollList = self.makeScrollList(gui, (0.55000000000000004, 0, 0.20200000000000001), (0.80000000000000004, 0.80000000000000004, 1, 1), self.allSuffixes, self.makeLabel, [
            TextNode.ALeft,
            'suffix'])
        gui.removeNode()
    ...
    def promptTutorial(self):
        self.promptTutorialDialog = TTDialog.TTDialog(parent = aspect2dp, text = TTLocalizer.PromptTutorial, text_scale = 0.059999999999999998, text_align = TextNode.ACenter, text_wordwrap = 22, command = self._NameShop__openTutorialDialog, fadeScreen = 0.5, style = TTDialog.TwoChoice, buttonTextList = [
            TTLocalizer.MakeAToonEnterTutorial,
            TTLocalizer.MakeAToonSkipTutorial], button_text_scale = 0.059999999999999998, buttonPadSF = 5.5, sortOrder = NO_FADE_SORT_INDEX)

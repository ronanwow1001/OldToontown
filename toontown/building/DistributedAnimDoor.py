...
class DistributedAnimDoor(DistributedDoor.DistributedDoor):
    ...
    def enterOpening(self, ts):
        ...
        self.doorTrack = Parallel(SoundInterval(self.openSfx, node = rightDoor), Sequence(HprInterval(rightDoor, VBase3(0, 0, 0)), Wait(0.40000000000000002), LerpHprInterval(nodePath = rightDoor, duration = 0.59999999999999998, hpr = VBase3(h, 0, 0), startHpr = VBase3(0, 0, 0), blendType = 'easeInOut')), name = trackName)
    ...
    def exitDoorEnterOpening(self, ts):
        ...
        if not leftDoor.isEmpty():
            ...
            self.doorExitTrack = Parallel(SoundInterval(self.openSfx, node = leftDoor), Sequence(LerpHprInterval(nodePath = leftDoor, duration = 0.59999999999999998, hpr = VBase3(h, 0, 0), startHpr = VBase3(0, 0, 0), blendType = 'easeInOut')), name = trackName)

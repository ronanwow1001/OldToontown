...
class TownBattleSOSPanel(DirectFrame, StateData.StateData):
    ...
    def load(self):
        ...
        self.title = DirectLabel(parent = self, relief = None, text = TTLocalizer.TownBattleSOSNoFriends, text_scale = 0.40000000000000002, text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1), pos = (0.0, 0.0, 1.5))
        ...
        self.close = DirectButton(parent = self, relief = None, image = (backGui.find('**/PckMn_BackBtn'), backGui.find('**/PckMn_BackBtn_Dn'), backGui.find('**/PckMn_BackBtn_Rlvr')), pos = (2.2999999999999998, 0.0, -1.6499999999999999), scale = 3, text = TTLocalizer.TownBattleSOSBack, text_scale = 0.050000000000000003, text_pos = (0.01, -0.012), text_fg = Vec4(0, 0, 0.80000000000000004, 1), command = self._TownBattleSOSPanel__close)
        gui.removeNode()
    ...
    def _TownBattleSOSPanel__updateNPCFriendsPanel(self):
        ...
        for (friend, count) in base.localAvatar.NPCFriendsDict.items():
            ...
            if (track == ToontownBattleGlobals.LURE_TRACK or self.canLure == 0 or track == ToontownBattleGlobals.TRAP_TRACK) and self.canTrap == 0:
    ...
    def _TownBattleSOSPanel__friendOnline(self, doId, commonChatFlags, whitelistChatFlags):
        self._TownBattleSOSPanel__updateScrollList()
        self._TownBattleSOSPanel__updateTitleText()

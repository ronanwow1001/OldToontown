...
class NPCMoviePlayer(DirectObject.DirectObject):
        ...
        for line in lines:
            ...
            if self.isLocalToon:
                if command == 'LOAD':
                    ...
                    iList.append(Func(self.acceptOnce, nextEvent, handleEvent, [
                        proceed]))
                    self.closePreviousChapter(iList)
                    chapterList = []
                    self.currentEvent = nextEvent
    ...
    def parseThrowSquirtPreview(self, line):
        ...
        return Sequence(Func(grabCurTrackAccess), LerpFunctionInterval(updateGagLevel, fromData = 1, toData = 7, duration = 0.29999999999999999), WaitInterval(3.5), LerpFunctionInterval(updateGagLevel, fromData = 7, toData = 1, duration = 0.29999999999999999), Func(restoreTrackAccess), Func(messenger.send, 'doneThrowSquirtPreview'))

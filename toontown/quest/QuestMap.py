...
class QuestMap(DirectFrame):
    ...
    def load(self):
        ...
        iconNP.removeNode()
        self.mapOpenButton = DirectButton(image = (gui.find('**/tt_t_gui_qst_mapClose'), gui.find('**/tt_t_gui_qst_mapClose'), gui.find('**/tt_t_gui_qst_mapTryToOpen')), relief = None, pos = (1.25, 0, -0.63), scale = 0.20499999999999999, command = self.show)
        self.mapCloseButton = DirectButton(image = (gui.find('**/tt_t_gui_qst_mapOpen'), gui.find('**/tt_t_gui_qst_mapOpen'), gui.find('**/tt_t_gui_qst_mapTryToClose')), relief = None, pos = (1.25, 0, -0.63), scale = 0.20499999999999999, command = self.hide)
        ...
        cogInfoTextColor = (0.20000000000000001, 0.20000000000000001, 0.20000000000000001, 1)
        textPos = (1.2, -0.20000000000000001)
        textScale = 0.80000000000000004
        self.cInfo = DirectLabel(parent = self.cogInfoFrame, text = '', text_fg = cogInfoTextColor, text_pos = textPos, text_scale = textScale, geom = cIcon, geom_pos = (-0.20000000000000001, 0, 0), geom_scale = 0.80000000000000004, relief = None)
        self.cInfo.setPos(-2.2000000000000002, 0, 0.5)
        self.lInfo = DirectLabel(parent = self.cogInfoFrame, text_fg = cogInfoTextColor, text = '', text_pos = textPos, text_scale = textScale, geom = lIcon, geom_pos = (-0.20000000000000001, 0, 0), geom_scale = 0.80000000000000004, relief = None)
        self.lInfo.setPos(-2.2000000000000002, 0, -0.5)
        self.mInfo = DirectLabel(parent = self.cogInfoFrame, text_fg = cogInfoTextColor, text = '', text_pos = textPos, text_scale = textScale, geom = mIcon, geom_pos = (-0.20000000000000001, 0, 0), geom_scale = 0.80000000000000004, relief = None)
        self.mInfo.setPos(0.80000000000000004, 0, 0.5)
        self.sInfo = DirectLabel(parent = self.cogInfoFrame, text_fg = cogInfoTextColor, text = '', text_pos = textPos, text_scale = textScale, geom = sIcon, geom_pos = (-0.20000000000000001, 0, 0), geom_scale = 0.80000000000000004, relief = None)
    ...
    def putBuildingMarker(self, pos, hpr = (0, 0, 0), mapIndex = None):
        marker = DirectLabel(parent = self.container, text = '', text_pos = (-0.050000000000000003, -0.14999999999999999), text_fg = (1, 1, 1, 1), relief = None)
    ...
    def update(self, task):
        ...
        for buildingMarker in self.buildingMarkers:
            buildingMarker.setScale((math.sin(task.time * 16.0 + i * math.pi / 3.0) + 1) * 0.0050000000000000001 + 0.040000000000000001)
            i = i + 1
    ...
    def initMarker(self, task):
        if self.av:
            ...
            self.marker['geom_scale'] = (1.3999999999999999 * task.time % 0.5) * 10 + 1
            self.marker['geom_color'] = (1, 1, 1, (0.80000000000000004 - (1.3999999999999999 * task.time % 0.5) * 2 / 0.80000000000000004) + 0.20000000000000001)

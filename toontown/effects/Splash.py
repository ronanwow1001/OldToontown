...
class Splash(NodePath):
    ...
    def createTrack(self, rate = 1):
        ...
        if self.wantParticles:
            particleSequence = Sequence(Func(self.pSystem.show), Func(self.particles.induceLabor), Func(self.pSystem.start, self), Wait(2.2000000000000002), Func(self.pSystem.hide), Func(self.pSystem.disable))
        else:

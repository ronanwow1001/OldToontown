    ...
    def generateFireworkIval(self):
        if not self.fireworkIval:
            ...
            if self.typeId == FireworkType.BasicPeony:
                ...
                leftEar = FireworkEffect(FireworkBurstType.PeonyShell, FireworkTrailType.Glow, velocity = Vec3(-25 * self.scale, 0, 115 * self.scale), scale = self.scale / 1.6000000000000001, primaryColor = self.primaryColor, secondaryColor = self.secondaryColor, burstDelay = 1.7)
                rightEar = FireworkEffect(FireworkBurstType.PeonyShell, FireworkTrailType.Glow, velocity = Vec3(25 * self.scale, 0, 115 * self.scale), scale = self.scale / 1.6000000000000001, primaryColor = self.primaryColor, secondaryColor = self.secondaryColor, burstDelay = 1.7)

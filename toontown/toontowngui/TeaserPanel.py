...
class TeaserPanel(DirectObject):
    ...
    def showPage(self, pageName):
        ...
        self.cleanup()
        self.dialog = TTDialog.TTDialog(parent = aspect2dp, text = TTLocalizer.TeaserTop, text_scale = TTLocalizer.TPdialog, text_align = TextNode.ACenter, text_wordwrap = TTLocalizer.TPdialogWordwrap, topPad = -0.14999999999999999, midPad = 1.25, sidePad = 0.25, pad = (0.25, 0.25), command = self._TeaserPanel__handleDone, fadeScreen = 0.5, style = TTDialog.TwoChoice, buttonTextList = [
            TTLocalizer.TeaserSubscribe,
            TTLocalizer.TeaserContinue], button_text_scale = TTLocalizer.TPbuttonTextList, buttonPadSF = 5.5, sortOrder = NO_FADE_SORT_INDEX, image = self.upsellBackground)
...
class FeatureBrowser(DirectScrolledList):
    ...
    def __init__(self, parent = aspect2dp, **kw):
        self.parent = parent
        optiondefs = (('parent', self.parent, None), ('relief', None, None), ('numItemsVisible', 1, None), ('items', [], None))
    ...
    def load(self):
        ...
        for page in PageOrder:
            ...
            panel = DirectFrame(parent = self, relief = None, image = guiModel, image_scale = (0.65000000000000002, 0.65000000000000002, 0.65000000000000002), image_pos = (0, 0, 0.0), text_align = TextNode.ACenter, text = textInfo, text_scale = TTLocalizer.TPpanel, text_pos = TTLocalizer.TPpanelPos)
            self.addItem(panel)

...
class ToontownLoadingBlocker(TTDialog.TTDialog):
    ...
    def __init__(self, avList):
        ...
        self.accept('phaseComplete-4', self._ToontownLoadingBlocker__shrinkLoadingBar)
        self.accept('launcherPercentPhaseComplete', self._ToontownLoadingBlocker__update)
    ...
    def _ToontownLoadingBlocker__setupLoadingBar(self):
        self.bar = DirectWaitBar(parent = self, guiId = 'DownloadBlockerBar', pos = (0, 0, -0.31380000000000002), relief = DGG.SUNKEN, frameSize = (-0.59999999999999998, 0.59999999999999998, -0.10000000000000001, 0.10000000000000001), borderWidth = (0.02, 0.02), scale = (0.80000000000000004, 0.80000000000000004, 0.5), range = 100, sortOrder = 5000, frameColor = (0.5, 0.5, 0.5, 0.5), barColor = (0.20000000000000001, 0.69999999999999996, 0.20000000000000001, 0.5), text = '0%', text_scale = (0.080000000000000002, 0.128), text_fg = (1, 1, 1, 1), text_align = TextNode.ACenter, text_pos = (0, -0.035000000000000003))
    ...
    def _ToontownLoadingBlocker__createTitleText(self):
        self.title = DirectLabel(parent = self, relief = None, guiId = 'BlockerTitle', pos = (0, 0, 0.38), text = TTLocalizer.BlockerTitle, text_font = ToontownGlobals.getSignFont(), text_fg = (1, 0.90000000000000002, 0.10000000000000001, 1), text_align = TextNode.ACenter, text_scale = 0.10000000000000001, textMayChange = 1, sortOrder = 50)

    
    def _ToontownLoadingBlocker__createLoadingText(self):
        self.loadingText = DirectLabel(parent = self, relief = None, guiId = 'BlockerLoadingText', pos = (0, 0, -0.23569999999999999), text = '', text_fg = (1, 1, 1, 1), text_scale = 0.055, textMayChange = 1, text_align = TextNode.ACenter, sortOrder = 50)
        self.loadingTextList = TTLocalizer.BlockerLoadingTexts
        self._ToontownLoadingBlocker__changeLoadingText()
        taskMgr.doMethodLater(self.loadingTextChangeTimer, self._ToontownLoadingBlocker__changeLoadingTextTask, 'changeLoadingTextTask')
    ...
    def _ToontownLoadingBlocker__changeLoadingText(self):
        ...
        if self.canChangeLoadingText:
            ...
            taskMgr.doMethodLater(self.loadingTextFreezeTime, self._ToontownLoadingBlocker__canChangeLoadingTextTask, 'canChangeLoadingTextTask')
    ...
    def _ToontownLoadingBlocker__createToonTip(self):
        self.toonTipText = DirectLabel(parent = self, relief = None, guiId = 'BlockerToonTip', pos = (0, 0, -0.46879999999999999), text = '', text_fg = (1, 1, 1, 1), text_scale = 0.050000000000000003, textMayChange = 1, text_align = TextNode.ACenter, text_wordwrap = 32, sortOrder = 50)
        self._ToontownLoadingBlocker__changeToonTip()
        taskMgr.doMethodLater(self.toonTipChangeTimer, self._ToontownLoadingBlocker__changeToonTipTask, 'changeToonTipTask')
    ...
    def _ToontownLoadingBlocker__shrinkLoadingBar(self):
        if self._ToontownLoadingBlocker__isValidDownloadBar():
            ivalDuration = 0.5
            barPosIval = LerpPosInterval(self.bar, ivalDuration, (-0.81000000000000005, 0, -0.95999999999999996))
            ...
            textScaleIval = LerpFunc(scaleText, fromData = 0.080000000000000002, toData = 0.16, duration = ivalDuration)
            textPosIval = LerpFunc(posText, fromData = -0.035000000000000003, toData = -0.050000000000000003, duration = ivalDuration)
            shrinkIval = Parallel(barPosIval, barScaleIval, textPosIval, textScaleIval, Func(self.loadingText.hide))
            self.hideBlockerIval = Sequence(shrinkIval, Wait(0.5), Func(self._ToontownLoadingBlocker__hideBlocker), Func(self._ToontownLoadingBlocker__resetLoadingBar), Func(self.destroy))
    ...
    def _ToontownLoadingBlocker__update(self, phase, percent, reqByteRate, actualByteRate):
        if self._ToontownLoadingBlocker__isValidDownloadBar():

...
class ChineseTutorial(DirectFrame, FSM.FSM):
    ...
    def __init__(self, doneFunction, doneEvent = None, callback = None):
        ...
        self.setStateArray([
            ...
            'Quit'])
        base.localAvatar.startSleepWatch(self.handleQuit)
        DirectFrame.__init__(self, pos = (-0.69999999999999996, 0.0, 0.0), image_color = ToontownGlobals.GlobalDialogColor, image_scale = (1.0, 1.5, 1.0), text = '', text_scale = 0.059999999999999998)
        self.accept('stoppedAsleep', self.handleQuit)
        self['image'] = DGG.getDefaultDialogGeom()
        self.title = DirectLabel(self, relief = None, text = '', text_pos = (0.0, 0.40000000000000002), text_fg = (1, 0, 0, 1), text_scale = 0.13, text_font = ToontownGlobals.getSignFont())
        ...
        self.bNext = DirectButton(self, image = (gui.find('**/Horiz_Arrow_UP'), gui.find('**/Horiz_Arrow_DN'), gui.find('**/Horiz_Arrow_Rllvr'), gui.find('**/Horiz_Arrow_UP')), image3_color = Vec4(1, 1, 1, 0.5), relief = None, text = TTLocalizer.ChineseTutorialNext, text3_fg = Vec4(0, 0, 0, 0.5), text_scale = 0.050000000000000003, text_pos = (0.0, -0.10000000000000001), pos = (0.34999999999999998, -0.29999999999999999, -0.33000000000000002), command = self.requestNext)
        self.bPrev = DirectButton(self, image = (gui.find('**/Horiz_Arrow_UP'), gui.find('**/Horiz_Arrow_DN'), gui.find('**/Horiz_Arrow_Rllvr'), gui.find('**/Horiz_Arrow_UP')), image3_color = Vec4(1, 1, 1, 0.5), image_scale = (-1.0, 1.0, 1.0), relief = None, text = TTLocalizer.ChineseTutorialPrev, text3_fg = Vec4(0, 0, 0, 0.5), text_scale = 0.050000000000000003, text_pos = (0.0, -0.10000000000000001), pos = (-0.34999999999999998, -0.29999999999999999, -0.33000000000000002), command = self.requestPrev)
        self.bQuit = DirectButton(self, image = (buttons.find('**/ChtBx_OKBtn_UP'), buttons.find('**/ChtBx_OKBtn_DN'), buttons.find('**/ChtBx_OKBtn_Rllvr')), relief = None, text = TTLocalizer.ChineseTutorialDone, text_scale = 0.050000000000000003, text_pos = (0.0, -0.10000000000000001), pos = (0.0, -0.29999999999999999, -0.33000000000000002), command = self.handleQuit)
        self.bQuit.hide()
        buttons.removeNode()
        gui.removeNode()
        self.request('Page1')
    ...
    def enterPage1(self, *args):
        ...
        self.blinker.append(LerpColorInterval(obj, 0.5, Vec4(0.5, 0.5, 0, 0.0), Vec4(0.20000000000000001, 0.20000000000000001, 0.20000000000000001, 1)))
        self.blinker.append(LerpColorInterval(obj, 0.5, Vec4(0.20000000000000001, 0.20000000000000001, 0.20000000000000001, 1), Vec4(0.5, 0.5, 0, 0.0)))
        self.blinker.loop()
...
class CheckersTutorial(DirectFrame, FSM.FSM):
    ...
    def enterPage3(self, *args):
        ...
        self.bNext['state'] = DGG.DISABLED
        self.blinker = Sequence()
        self.blinker.append(LerpColorInterval(self.obj, 0.5, Vec4(0.5, 0.5, 0, 0.0), Vec4(0.90000000000000002, 0.90000000000000002, 0, 1)))
        self.blinker.append(LerpColorInterval(self.obj, 0.5, Vec4(0.90000000000000002, 0.90000000000000002, 0, 1), Vec4(0.5, 0.5, 0, 0.0)))
        self.blinker.loop()
        self.bNext.hide()
        self.iPage3.show()
        self.iPage4.show()

...
class OZPlayground(Playground.Playground):
    ...
    def __init__(self, loader, parentFSM, doneEvent):
        ...
        self.fsm.addState(State.State('picnicBasketBlock', self.enterPicnicBasketBlock, self.exitPicnicBasketBlock, [
            'walk']))
    ...
    def enterStart(self):
        ...
        taskMgr.add(self._OZPlayground__checkToonUnderwater, 'oz-check-toon-underwater')
        taskMgr.add(self._OZPlayground__checkCameraUnderwater, 'oz-check-cam-underwater')
    ...
    def enterTeleportIn(self, requestStatus):
        ...
        if reason == RaceGlobals.Exit_Barrier:
            ...
            self.dialog = TTDialog.TTDialog(text = TTLocalizer.KartRace_RaceTimeout, command = self._OZPlayground__cleanupDialog, style = TTDialog.Acknowledge)
        elif reason == RaceGlobals.Exit_Slow:
            requestStatus['nextState'] = 'popup'
            self.dialog = TTDialog.TTDialog(text = TTLocalizer.KartRace_RacerTooSlow, command = self._OZPlayground__cleanupDialog, style = TTDialog.Acknowledge)
        elif reason == RaceGlobals.Exit_BarrierNoRefund:
            requestStatus['nextState'] = 'popup'
            self.dialog = TTDialog.TTDialog(text = TTLocalizer.KartRace_RaceTimeoutNoRefund, command = self._OZPlayground__cleanupDialog, style = TTDialog.Acknowledge)

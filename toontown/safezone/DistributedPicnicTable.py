...
class DistributedPicnicTable(DistributedNode.DistributedNode):
    ...
    def setTableState(self, tableStateList, isplaying):
        ...
        if self.gameMenu:
            if numPlayers > 2:
                print ' GETTING HERE!!'
                self.gameMenu.FindFour.setColor(0.69999999999999996, 0.69999999999999996, 0.69999999999999996, 0.69999999999999996)
                self.gameMenu.FindFour['command'] = self.doNothing
                self.gameMenu.findFourText['fg'] = (0.69999999999999996, 0.69999999999999996, 0.69999999999999996, 0.69999999999999996)
                self.gameMenu.Checkers.setColor(0.69999999999999996, 0.69999999999999996, 0.69999999999999996, 0.69999999999999996)
                self.gameMenu.Checkers['command'] = self.doNothing
                self.gameMenu.checkersText['fg'] = (0.69999999999999996, 0.69999999999999996, 0.69999999999999996, 0.69999999999999996)
    ...
    def enableChoiceButtons(self):
        if self.tableState[self.seatBumpForObserve] == None and self.isPlaying == False:
            self.joinButton = DirectButton(relief = None, text = TTLocalizer.PicnicTableJoinButton, text_fg = (1, 1, 0.65000000000000002, 1), text_pos = (0, -0.23000000000000001), text_scale = 0.80000000000000004, image = (self.upButton, self.downButton, self.rolloverButton), image_color = (1, 0, 0, 1), image_scale = (20, 1, 11), pos = (0, 0, 0.80000000000000004), scale = 0.14999999999999999, command = lambda self = self: self.joinButtonPushed())
        
        if self.isPlaying == True:
            self.observeButton = DirectButton(relief = None, text = TTLocalizer.PicnicTableObserveButton, text_fg = (1, 1, 0.65000000000000002, 1), text_pos = (0, -0.23000000000000001), text_scale = 0.80000000000000004, image = (self.upButton, self.downButton, self.rolloverButton), image_color = (1, 0, 0, 1), image_scale = (20, 1, 11), pos = (0, 0, 0.59999999999999998), scale = 0.14999999999999999, command = lambda self = self: self.observeButtonPushed())
        
        self.exitButton = DirectButton(relief = None, text = TTLocalizer.PicnicTableCancelButton, text_fg = (1, 1, 0.65000000000000002, 1), text_pos = (0, -0.23000000000000001), text_scale = 0.80000000000000004, image = (self.upButton, self.downButton, self.rolloverButton), image_color = (1, 0, 0, 1), image_scale = (20, 1, 11), pos = (1, 0, 0.59999999999999998), scale = 0.14999999999999999, command = lambda self = self: self.cancelButtonPushed())
        self.tutorialButton = DirectButton(relief = None, text = TTLocalizer.PicnicTableTutorial, text_fg = (1, 1, 0.65000000000000002, 1), text_pos = (-0.050000000000000003, -0.13), text_scale = 0.55000000000000004, image = (self.upButton, self.downButton, self.rolloverButton), image_color = (1, 0, 0, 1), image_scale = (20, 1, 11), pos = (-1, 0, 0.59999999999999998), scale = 0.14999999999999999, command = lambda self = self: self.tutorialButtonPushed())
        ...
        if self.cr.doId2do.has_key(avId):
            ...
            if avId == base.localAvatar.getDoId():
                if not base.cr.playGame.getPlace() == None:
                    ...
                    track.append(Func(self._DistributedPicnicTable__disableCollisions))
    ...
    def emptySlot(self, avId, index, timestamp):
        ...
        if self.cr.doId2do.has_key(avId):
            ...
            if base.localAvatar.getDoId() == avId:
                self.outTrack.append(Func(self._DistributedPicnicTable__enableCollisions))
    ...
    def enableStopObserveButton(self):
        self.stopObserveButton = DirectButton(relief = None, text = 'Stop Observing', text_fg = (1, 1, 0.65000000000000002, 1), text_pos = (0, -0.23000000000000001), text_scale = 0.45000000000000001, image = (self.upButton, self.downButton, self.rolloverButton), image_color = (1, 0, 0, 1), image_scale = (20, 1, 11), pos = (0.92000000000000004, 0, 0.40000000000000002), scale = 0.14999999999999999, command = lambda self = self: self.stopObserveButtonPushed())
    ...
    def generateToonJumpTrack(self, av, seatIndex):
        ...
        def getToonJumpTrack(av, seatIndex):
            ...
            toonJumpTrack = Parallel(ActorInterval(av, 'jump'), Sequence(Wait(0.42999999999999999), Parallel(LerpHprInterval(av, hpr = getJumpHpr, duration = 1), ProjectileInterval(av, endPos = getJumpDest, duration = 1))))
            return toonJumpTrack
    ...
    def clearToonTracks(self):
        ...
        for key in keyList:
            if self._DistributedPicnicTable__toonTracks.has_key(key):

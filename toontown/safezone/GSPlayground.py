...
class GSPlayground(Playground.Playground):
    ...
    def __init__(self, loader, parentFSM, doneEvent):
        ...
        self.fsm.addState(State.State('startingBlock', self.enterStartingBlock, self.exitStartingBlock, [
            'walk']))
    ...
    def enterTeleportIn(self, requestStatus):
        ...
        if reason == RaceGlobals.Exit_Barrier:
            ...
            self.dialog = TTDialog.TTDialog(text = TTLocalizer.KartRace_RaceTimeout, command = self._GSPlayground__cleanupDialog, style = TTDialog.Acknowledge)
        elif reason == RaceGlobals.Exit_Slow:
            requestStatus['nextState'] = 'popup'
            self.dialog = TTDialog.TTDialog(text = TTLocalizer.KartRace_RacerTooSlow, command = self._GSPlayground__cleanupDialog, style = TTDialog.Acknowledge)
        elif reason == RaceGlobals.Exit_BarrierNoRefund:
            requestStatus['nextState'] = 'popup'
            self.dialog = TTDialog.TTDialog(text = TTLocalizer.KartRace_RaceTimeoutNoRefund, command = self._GSPlayground__cleanupDialog, style = TTDialog.Acknowledge)

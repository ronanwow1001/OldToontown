...
class DistributedCheckers(DistributedNode.DistributedNode):
    ...
    def __init__(self, cr):
        ...
        for x in range(0, 32):
            ...
            tempList[x].node().addSolid(CollisionSphere(0, 0, 0, 0.39000000000000001))
    ...
    def handleClicked(self, index):
        ...
        if self.moveList == []:
            ...
            self.blinker.append(LerpColorInterval(self.locatorList[index], 0.69999999999999996, self.tintConstant, col))
            self.blinker.append(LerpColorInterval(self.locatorList[index], 0.69999999999999996, col, self.tintConstant))
            self.blinker.loop()
            self.sound.start()

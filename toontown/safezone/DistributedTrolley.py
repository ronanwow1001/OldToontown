...
class DistributedTrolley(DistributedObject.DistributedObject):
    ...
    def fillSlot(self, index, avId):
        ...
        if self.cr.doId2do.has_key(avId):
            ...
            track = Sequence(LerpPosInterval(toon, TOON_BOARD_TIME * 0.75, Point3(-5, -4.5 + index * 3, 1.3999999999999999)), LerpHprInterval(toon, TOON_BOARD_TIME * 0.25, Point3(90, 0, 0)), Parallel(Sequence(Wait(sitStartDuration * 0.25), LerpPosInterval(toon, sitStartDuration * 0.25, Point3(-3.8999999999999999, -4.5 + index * 3, 3.0))), ActorInterval(toon, 'sit-start')), Func(toon.setAnimState, 'Sit', 1.0), Func(self.clearToonTrack, avId), name = toon.uniqueName('fillTrolley'), autoPause = 1)
    ...
    def clearToonTracks(self):
        ...
        for key in keyList:
            if self._DistributedTrolley__toonTracks.has_key(key):

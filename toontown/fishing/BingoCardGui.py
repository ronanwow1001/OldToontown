...
class BingoCardGui(DirectFrame):
    ...
    def __init__(self, parent = aspect2d, **kw):
        ...
        optiondefs = (('relief', None, None), ('state', DGG.NORMAL, None), ('image', self.model.find('**/g'), None), ('image_color', BG.getColor(0), None), ('image_scale', BG.CardImageScale, None), ('image_hpr', (0.0, 90.0, 0.0), None), ('pos', BG.CardPosition, None))
    ...
    def loadNextGameTimer(self):
        ...
        self.nextGameTimer.setProp('text_scale', 0.20000000000000001 * BG.CardImageScale[2])
    ...
    def load(self):
        ...
        self.bingo = DirectButton(parent = self, pos = (BG.GridXOffset, 0, 0.30499999999999999), scale = (0.034299999999999997, 0.035000000000000003, 0.035000000000000003), relief = None, state = DGG.DISABLED, geom = self.model.find('**/BINGObutton'), geom_pos = (0, 0, 0), geom_hpr = (0, 90, 0), image = (self.model.find('**/Gold_TTButtUP'), self.model.find('**/goldTTButtDown'), self.model.find('**/RolloverBingoButton1')), image_pos = (0, 0, 0), image_hpr = (0, 90, 0), image_color = BG.getButtonColor(0), pressEffect = False)
        guiButton.removeNode()
        arrowModel = loader.loadModel('phase_3.5/models/gui/speedChatGui')
        self.gameType = DirectButton(parent = self, pos = (BG.GridXOffset, 0, -8 * BG.CardImageScale[2] - 0.01), relief = None, image = arrowModel.find('**/chatArrow'), image_scale = -0.050000000000000003, image_pos = (-0.20000000000000001, 0, 0.025000000000000001), text = '', text_scale = 0.044999999999999998, text_fg = (1, 1, 1, 1), text_font = ToontownGlobals.getSignFont(), text_wordwrap = 10.5, text_pos = (0.01, 0.0080000000000000002), pressEffect = False)
        ...
        self.jpSign = DirectFrame(parent = self.parent, relief = None, state = DGG.NORMAL, pos = BG.CardPosition, scale = (0.035000000000000003, 0.035000000000000003, 0.035000000000000003), text = TTLocalizer.FishBingoJackpot, text_scale = 2, text_pos = (-1.5, 18.600000000000001), text_fg = (1, 1, 1, 1), image = self.model.find('**/jackpot'), image_pos = (0, 0, 0), image_hpr = (0, 90, 0), sortOrder = DGG.BACKGROUND_SORT_INDEX)
    ...
    def _BingoCardGui__indicateMatches(self, bFlipFlop, fish):
        ...
        if bFlipFlop:
            ...
            self.resetCellColors()
        taskMgr.doMethodLater(0.5, self._BingoCardGui__indicateMatches, self.taskNameFlashFish, extraArgs = (not bFlipFlop, fish))
        return Task.done
    ...
    def _BingoCardGui__indicateBingo(self, bFlipFlop):
        ...
        if bFlipFlop:
            ...
            c = BG.getButtonColor(self.game.getGameType())
            self.bingo['image_color'] = Vec4(c[0], c[1], c[2], c[3])
        taskMgr.doMethodLater(0.5, self._BingoCardGui__indicateBingo, 'bingoFlash', extraArgs = (not bFlipFlop,))
        return Task.done
    ...
    def makeTutorial(self):
        self.tutorial = TTDialog.TTDialog(fadeScreen = 0, pad = (0.050000000000000003, 0.050000000000000003), midPad = 0, topPad = 0, sidePad = 0, text = TTLocalizer.FishBingoHelpBlockout, style = TTDialog.NoButtons, pos = BG.TutorialPosition, scale = BG.TutorialScale)
        self.tutorial.hide()

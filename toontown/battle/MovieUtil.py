...
def createSuitReviveTrack(suit, toon, battle, npcs = []):
    ...
    deathSoundTrack = Sequence(Wait(0.80000000000000004), SoundInterval(spinningSound, duration = 1.2, startTime = 1.5, volume = 0.20000000000000001, node = suit), SoundInterval(spinningSound, duration = 3.0, startTime = 0.59999999999999998, volume = 0.80000000000000004, node = suit), SoundInterval(deathSound, volume = 0.32000000000000001, node = suit))
    ...
    gears1Track = Sequence(Wait(2.1000000000000001), ParticleInterval(smallGears, battle, worldRelative = 0, duration = 4.2999999999999998, cleanup = True), name = 'gears1Track')
    gears2MTrack = Track((0.0, explosionTrack), (0.69999999999999996, ParticleInterval(singleGear, battle, worldRelative = 0, duration = 5.7000000000000002, cleanup = True)), (5.2000000000000002, ParticleInterval(smallGearExplosion, battle, worldRelative = 0, duration = 1.2, cleanup = True)), (5.4000000000000004, ParticleInterval(bigGearExplosion, battle, worldRelative = 0, duration = 1.0, cleanup = True)), name = 'gears2MTrack')
...
def createSuitTeaseMultiTrack(suit, delay = 0.01):
    suitTrack = Sequence(Wait(delay), ActorInterval(suit, 'victory', startTime = 0.5, endTime = 1.8999999999999999), Func(suit.loop, 'neutral'))
    missedTrack = Sequence(Wait(delay + 0.20000000000000001), Func(indicateMissed, suit, 0.90000000000000002))
...
def createKapowExplosionTrack(parent, explosionPoint = None, scale = 1.0):
    ...
    explosionTrack.append(Func(explosion.setScale, 0.40000000000000002 * scale))

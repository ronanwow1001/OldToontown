...
def doEffect(textId, speakingToon, nearbyToons):
    ...
    for toonId in nearbyToons:
        ...
        if toon and not (toon.ghostMode):
            i = Sequence(toon.doToonColorScale(fadeColor, 0.29999999999999999), toon.doToonColorScale(toon.defaultColorScale, 0.29999999999999999), Func(toon.restoreDefaultColorScale))
            recolorToons.append(i)
            continue
    
    i = Parallel(ParticleInterval(effect, speakingToon, worldRelative = 0, duration = 3, cleanup = True), Sequence(Wait(0.20000000000000001), recolorToons), autoFinish = 1)
    i.start()

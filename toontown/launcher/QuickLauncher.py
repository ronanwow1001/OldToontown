...
class QuickLauncher(LauncherBase):
    ...
    def downloadLauncherFileDbDone(self):
        ...
        for fileDesc in fileList.split():
            ...
            self.notify.info('fileName: %s, flag:=%s directions=%s, extract=%s required=%s optDownload=%s' % (fileName, flag, directions, extract, required, optionalDownload))
            if required:
    ...
    def resumeInstall(self):
        for (curVer, expectedSize, expectedMd5) in range(len(self.LauncherPhases)):
